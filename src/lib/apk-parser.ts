...
async function analyze(file: File): Promise<ParsedApk> {
  ...
  return {
    ...
    versionCode,
    minSdk: toInt(usesSdk?.attrs["minSdkVersion"]?.value ?? null) ?? usesSdk?.attrs["minSdkVersion"]?.raw ?? null,
    targetSdk: toInt(usesSdk?.attrs["targetSdkVersion"]?.value ?? null) ?? usesSdk?.attrs["targetSdkVersion"]?.raw ?? null,
